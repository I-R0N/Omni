...
export class GameEngine {
  ...
  /**
   * Transition to CLIENT mode.  Loads a fresh UniverseMap for local visual
   * scenery (asteroids + tile landmarks) since the snapshot wire format
   * strips static entities to stay under the iOS data channel ceiling.
   * The scenery won't match the host's exactly (different random seeds)
   * but provides visual texture and populates the minimap.  Dynamic
   * entities come in via applySnapshot each tick.
   */
  ...
  public applySnapshot(snap: SnapshotMessage) {
    ...
    // Static entities (asteroids, tile structures) are client-local scenery
    // — they're generated by UniverseMap.init() when the client enters
    // CLIENT mode and are never sent over the wire because the full map is
    // far too large to fit in a single iOS Safari data channel message.
    // Preserve them across snapshot applies; only dynamic entities
    // (players, enemies, projectiles, drops) are merged from the wire.
    const ents = this.currentMap.entities;
    const existing = new Map<string, GameEntity>();
    const staticEnts: GameEntity[] = [];
    for (let i = 0; i < ents.length; i++) {
      const e = ents[i];
    ...
    // Also index this.player so applySerializedEntity can update it in place
    // when the self entity comes through.
    existing.set(this.player.id, this.player);
    if (this.player2) existing.set(this.player2.id, this.player2);

    const nextDynamic: GameEntity[] = [];
    ...
    for (let i = 0; i < snap.entities.length; i++) {
      ...
      ent.id = se.id;
      nextDynamic.push(ent);
      if (se.id === snap.yourId) {
        selfEntity = ent;
      }
    }
    // Reassemble: locally-generated scenery + dynamic entities from snapshot.
    this.currentMap.entities = staticEnts.concat(nextDynamic);
  ...
  private loop = (time: number) => {
    ...
    if (this.netRole === NetRole.CLIENT) {
        ...
        //
        // Skip static scenery (STRUCTURE, ASTEROID) — on the client those
        // are locally-generated visuals with no physics reanchoring, so any
        // initial flow-field velocity would carry them off-screen forever.
        ...
        if (this.currentMap) {
            ...
            for (let i = 0; i < ents.length; i++) {
                ...
                if (e.type === EntityType.STRUCTURE || e.type === EntityType.ASTEROID) continue;
