...
export class WaveSystem {
   ...
   * counted wave enemy — so the wave's clear-the-field completion rule already
   * gates on killing it, with no boss-specific completion plumbing.  The
   * caller's `onBossSpawn` hook fires the entrance rift VFX.
   */
  private spawnBoss(subtype: EnemySubtype, ctx: WaveSpawnContext): GameEntity {
    const id = nextId(`boss_${this.waveIndex}`);
    const { x, y } = this.findSpawnPoint(subtype, ctx);
    const boss = this.buildEnemy(id, subtype, x, y, ctx, true);
   ...
   *  the reference).  Shared by the stream spawn and the boss spawn. */
  ...
  private buildEnemy(
    ...
    const tierMap: Partial<Record<string, number>> = {
      ...
      // Bosses sit above the tier ladder — the tier only scales the kill
      // points, and the real payout is the BOSS_CONSTANTS burst on top.
      BOSS_SCATTER: 4, BOSS_SIEGE: 4,
    };
    ...
    // Boss ((h)): mark it so the HUD bar / aura / payout paths pick it up.  The
    // phase itself is applied by GameEngine.updateBosses on the first tick
    // (phase 0 falls out of the normal health-fraction check), which keeps ONE
    // code path responsible for phase state.
    if (BOSS_DEFS[subtype]) {
      enemy.isBoss = true;
      enemy.bossPhase = -1; // -1 = "no phase applied yet" → phase 0 stamps next tick
    }
...
export interface WaveSpawnContext {
   ...
   *  for the entrance rift VFX.  Absent → no VFX (headless / tests). */
