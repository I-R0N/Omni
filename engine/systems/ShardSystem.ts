...
/**
 * Persistent spring-elastic bond between two shard-family entities
 * (today: plastic-shard ↔ plastic-shard and plastic-shard ↔ plastic-
 * tile).  Separate lifecycle from BondEntry: elastic bonds form
 * once on contact and re-apply a Hooke's-law spring force every
 * tick until the pair drifts past `breakDist`, at which point the
 * bond breaks permanently.  Fields cached at formation so the
 * per-tick path doesn't re-resolve variants.
 */
interface ElasticBondEntry {
  a: GameEntity;
  b: GameEntity;
  /** Multiplier on (a.size.x + b.size.x) × 0.5 for the spring's
   *  rest length.  Resolved each tick because shard sizes can
   *  change (density compaction / shatter swaps). */
  restFactor: number;
  /** Multiplier on the resolved rest distance — beyond this the
   *  bond permanently breaks. */
  breakFactor: number;
...
/** Multiplier on `restFactor × contactDist` used as the formation
 *  threshold.  Bonds form when the pair is within this distance
 *  AND not already bonded.  1.5× rest gives newly-spawned shards
 *  room to bond with neighbours just outside the rest distance
 *  without the spring snapping shut. */
const ELASTIC_BOND_FORMATION_FACTOR = 1.5;

/** Order-independent pair key for the elastic-bond dedupe set. */
function elasticBondKey(idA: string, idB: string): string {
...
export class ShardSystem {
  ...
  /**
   * Persistent spring-elastic bonds — separate lifecycle from
   * `bonds` above.  Each entry stores cached spring parameters so
   * the per-tick path doesn't re-resolve the variant.  Today only
   * plastic-shard ↔ plastic-shard and plastic-shard ↔ plastic-tile
   * pairs populate this list.
   */
  private elasticBonds: ElasticBondEntry[] = [];
  /**
   * Pair-key index for elasticBonds — keyed by `min(a.id, b.id) +
   * '|' + max(a.id, b.id)`.  Used in the formation pass to dedupe
   * O(1) instead of scanning the whole bond list per candidate.
   * Cleared and rebuilt at the top of runElasticBondFormation; only
   * touched once per frame so the string allocations stay bounded.
   */
  private elasticBondKeys: Set<string> = new Set();
  ...
  public update(
    ...
    // Elastic-bond tick runs every frame regardless of runMergePass
    // — the spring force must integrate at every physics step or
    // bonded clusters drift apart between ShPair frames.  Cheap:
    // only iterates active bonds, no broadphase work here.
    this.tickElasticBonds(dt);
    if (runMergePass) {
      this.runMergeBroadphase(entities, dt, physics);
      this.runElasticBondFormation(entities);
  ...
  public reset(): void {
    ...
    this.bonds.length = 0;
    this.elasticBonds.length = 0;
    this.elasticBondKeys.clear();
  }
  ...
  // ── Elastic-bond pipeline (plastic-softbody retrofit) ────────────
  // Persistent spring-coupled pairs that integrate every tick.
  // Separate lifecycle from the bondsWith stick-bond pipeline:
  //   - bondsWith bonds resolve via a one-shot timer + transmute.
  //   - elastic bonds re-apply Hooke's-law force every tick until
  //     the pair drifts past the break threshold.
  //
  // The two pipelines coexist without interference — variants opt
  // in to either or both via their SHARD_VARIANTS entry (today
  // plastic-shard uses elasticBond only).

  /**
   * Per-tick spring integration for every active elastic bond.
   * Applies a Hooke's-law acceleration along the bond axis toward
   * the resolved rest distance, plus a damping term proportional
   * to relative velocity along the bond axis.  Bonds whose parties
   * have gone inactive, or whose dist exceeds the break threshold,
   * are dropped silently.
   *
   * Allocation discipline: no {x, y} objects allocated in the hot
   * path — the bond entry holds entity refs, and per-tick vectors
   * live in local scalars (dx / dy / nx / ny).  Compaction is the
   * standard write-index compaction pattern used elsewhere in
   * ShardSystem.
   */
  private tickElasticBonds(dt: number): void {
    if (this.elasticBonds.length === 0) return;

    let writeIdx = 0;
    for (let bi = 0; bi < this.elasticBonds.length; bi++) {
      const bond = this.elasticBonds[bi];
      const a = bond.a;
      const b = bond.b;

      if (!a.active || !b.active) continue; // discard inactive pair
      // Pairs in their merge-fade-out should not be force-pulled or
      // damped — they're in their death animation, the spring
      // would jerk them off-course while they dissolve.
      if (a.mergeFadeTimer !== undefined || b.mergeFadeTimer !== undefined) continue;

      // Toroidal-correct delta — naïve subtraction silently breaks
      // bonds straddling a seam.
      ...
      const restDist    = contactDist * bond.restFactor;
      const breakDist   = restDist    * bond.breakFactor;

      if (distSq > breakDist * breakDist) continue; // bond snaps permanently

      // Degenerate overlap (entities at the same position) — no
      // meaningful bond direction this frame; defer to next tick.
      if (distSq < 0.0001) {
      ...
      const nx = dx * invDist; // direction from a → b
      const ny = dy * invDist;
      ...
      // bond reads as a constraint between the pair, not a true
      // mass-weighted spring.  Static tiles (mass = Infinity) are
      // unmovable and absorb their share of the impulse implicitly.
      const springAccel = bond.stiffness * stretch * dt;

      // Damping along the bond axis: pull relative velocity toward
      // zero along the bond direction so oscillations decay.
      ...
      // pulls a toward b and b toward a (compress), negative pushes
      // them apart.  Stretch is positive when extended → springAccel
      // is positive (compress), pulling them together.  Damping
      // adds a velocity-proportional pull that opposes relative
      // motion along the axis.
      //
      // Per-substep clamp: keeps the explicit-Euler integration
      // stable when stretches approach breakDist.  Without the
      // clamp, a bond about to break would deliver one huge
      // velocity update that teleports the shard past its partner
      // before the breakDist check on the next tick.  Capped at
      // 8 velocity units (≈ player projectile range) so the spring
      // still feels strong but never explodes.
      let accel = springAccel + dampAccel;
      const MAX_DV = 8;
      if (accel >  MAX_DV) accel =  MAX_DV;
      if (accel < -MAX_DV) accel = -MAX_DV;

      // Apply to each party.  Static tiles (mass = Infinity) skip
      // the update — the spring still pulls the mobile side toward
      // the fixed point, which is the desired softbody-around-tile
      // anchor behaviour.
      if (a.mass !== Infinity) {
        a.velocity.x += nx * accel;
        a.velocity.y += ny * accel;
      }
      if (b.mass !== Infinity) {
        b.velocity.x -= nx * accel;
        b.velocity.y -= ny * accel;
      }

      this.elasticBonds[writeIdx++] = bond;
    }
    this.elasticBonds.length = writeIdx;

    // Keep the dedupe set in sync — rebuild from the compacted
    // list so dropped bonds are removable in the next formation
    // pass.  Cheap: one walk over a short list.
    this.elasticBondKeys.clear();
    for (let i = 0; i < this.elasticBonds.length; i++) {
      const bond = this.elasticBonds[i];
      this.elasticBondKeys.add(elasticBondKey(bond.a.id, bond.b.id));
    }
  }

  /**
   * Form new elastic bonds for any plastic-family pair within
   * formation range that isn't already bonded.  Runs once per
   * merge frame (gated by runMergePass alongside the other
   * broadphase passes); the per-tick spring integration in
   * tickElasticBonds runs every frame regardless.
   *
   * Iteration: walks every active shard-family entity with an
   * `elasticBond` policy, scans a 3×3 spatial neighbourhood for
   * potential partners, and pushes one ElasticBondEntry per new
   * pair.  Either party may declare the bond via its `partners`
   * list; the formation pass treats both sides symmetrically.
   *
   * Cell size: same SPATIAL_GRID_SIZE as PhysicsSystem (120) so
   * the 3×3 scan covers up to ~360 units — comfortably above the
   * formation distance for typical plastic shard sizes (rest ×
   * 1.5 ≈ 80 units at 30 + 30 diameter pair).
   */
  private runElasticBondFormation(entities: GameEntity[]): void {
    // Bail early when no plastic-family entities are active —
    // avoids the candidate walk + spatial-hash build on every
    // map that doesn't use elasticBond.
    let anyPlastic = false;
    for (let i = 0; i < entities.length; i++) {
      const e = entities[i];
      if (!e.active) continue;
      if (e.shardVariant === undefined) continue;
      if (SHARD_VARIANTS[e.shardVariant].elasticBond !== undefined) {
        anyPlastic = true;
        break;
      }
    }
    if (!anyPlastic) return;

    // Candidate set: every entity whose variant has an elasticBond
    // policy OR is a valid partner for one.  We iterate plastic-
    // shards (the only variant with `elasticBond` set today) and
    // scan the spatial hash for partners — plastic-tiles get
    // bonded reactively when a plastic-shard finds them.
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      // Either has an elasticBond policy, or is a partner of one.
      // The cheapest correct set today is "any shard-family entity
      // matching a known elasticBond target" — but enumerating all
      // partners requires scanning SHARD_VARIANTS.  Today the only
      // policy is plastic-shard's [plastic-shard, plastic-tile],
      // so a hardcoded variant check is fine and avoids a second
      // pass.
    ...
    // Spatial hash keyed by SPATIAL_GRID_SIZE-sized cells.  Same
    // dimensions as PhysicsSystem's grid so the 3×3 scan radius
    // matches familiar cell math.
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      const aVariantId = a.shardVariant!;
      const aVariant = SHARD_VARIANTS[aVariantId];
      // Only the side(s) that declare elasticBond drive the pair
      // — but EITHER side may declare it, so check both.  We
      // start from candidates with elasticBond set (today: plastic-
      // shard); the other side (plastic-tile) is a valid partner
      // but doesn't declare the policy.
      const aElastic = aVariant.elasticBond;
      if (aElastic === undefined) continue;
      ...
      for (let ncx = acx - 1; ncx <= acx + 1; ncx++) {
        for (let ncy = acy - 1; ncy <= acy + 1; ncy++) {
          ...
          for (let k = 0; k < cell.length; k++) {
            const j = cell[k];
            // Skip self, and dedupe via j > i for same-elasticBond
            // ordering.  When the partner DOESN'T declare elasticBond
            // (e.g. plastic-tile), j may be < i — process anyway,
            // since the partner has no chance to iterate this pair
            // from its own side.
            if (j === i) continue;
            const b = candidates[j];
            const bVariantId = b.shardVariant!;
            const bElastic = SHARD_VARIANTS[bVariantId].elasticBond;
            // If both sides declare elasticBond, dedupe via id
            // ordering.  If only `a` declares it, process regardless.
            if (bElastic !== undefined && j <= i) continue;

            // Partner-list check from a's side.
            let matched = false;
            if (aElastic.partners.indexOf(bVariantId) !== -1) matched = true;
            else if (bElastic !== undefined
                && bElastic.partners.indexOf(aVariantId) !== -1) matched = true;
            if (!matched) continue;

            // Dedupe via the persistent pair-key set.
            const pairKey = elasticBondKey(a.id, b.id);
            if (this.elasticBondKeys.has(pairKey)) continue;

            // Formation distance gate — bond forms only when the
            // pair is within rest × ELASTIC_BOND_FORMATION_FACTOR.
            // Use a's elasticBond params as the canonical resolved
            // values (if both sides declare, the formation pass is
            // symmetric, but the tick reads only the cached entry).
            const dx = wrapDeltaX(a.position.x, b.position.x);
            const dy = wrapDeltaY(a.position.y, b.position.y);
            const distSq = dx * dx + dy * dy;
            const contactDist = (a.size.x + b.size.x) * 0.5;
            const formationDist = contactDist * aElastic.restFactor * ELASTIC_BOND_FORMATION_FACTOR;
            if (distSq > formationDist * formationDist) continue;

            this.elasticBonds.push({
              a, b,
              restFactor:  aElastic.restFactor,
              breakFactor: aElastic.breakFactor,
              stiffness:   aElastic.stiffness,
              damping:     aElastic.damping,
            });
            this.elasticBondKeys.add(pairKey);
          }
        }
      }
    }
