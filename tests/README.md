...
| `debugmenu.spec.ts` | 26 | **The debug panel** — one component on every screen, declared by `components/debugSections.tsx`. REACH: opened from the main menu, live play, pause, every station tab, the death screen and the stage-clear screen, with one row (Screen shake) toggled on each and the ENGINE flag read back, and the main-menu map chip still swapping the backdrop without starting a run. FREEZE: off by default (a planted probe keeps drifting), on holds the world and resumes on close, and — the load-bearing one — it NEVER holds a dying or dead ship: with the freeze on and the panel open, the explosion, the beat and the summary all still arrive (without that exclusion the death screen hangs forever; measured as a timeout) and the world keeps running behind the summary. INPUT, each with a positive CONTROL so the zeros mean something: mouse clicks and hovers on the panel fire nothing, steer nothing, turn nothing and take no keyboard focus; a TOUCH tap turns nothing (the browser's compatibility mousemove after a tap used to swing the ship toward the row — verified failing with the guard removed); keys typed into the filter — HELD, not just typed, since a quick type releases before a frame looks — neither fly nor dock the ship parked in dock range; ` and Escape (which clears the filter first); and a GAMEPAD answered through `navigator.getGamepads` so the engine's own poll runs the real capture: Select opens, the stick and R2 are captured, the D-pad focuses inside the panel and Cross presses, Circle closes WITHOUT banking a scan (verified failing without the drain), and over the pause menu BACK closes the panel first. PAYLOAD: the weapon catalog and slot-lock readout ride only while the panel is open. REGISTRY: every row label of the old menu survives (a multiset, byte for byte — the timing tree indents with no-break spaces), every old chip survives, every control answers a click with a clean console (the sweep that found `Outfit all` throwing on the deleted Penetration module), the filter finds rows by label, chip and tooltip, and open sections are remembered across closing and screens. DESCRIPTIONS: every control and chip carries a one-line summary no longer than 160 characters, over 100 rows keep their old tooltip as the detail behind "More", and no native `title` is left on a row; a MOUSE resting on a row shows it (not on arrival — the dwell; verified failing with the dwell at 0), moving into the popup keeps it so "More" works, leaving closes it; KEYBOARD / PAD focus shows it and swaps it as focus moves (verified failing when the body's scroll — which focusing a row causes — cancelled the pending open); and a TOUCH long-press on the Warden chip shows it WHILE the finger is down, keeps it after, and does NOT warp the boss in — the browser's release click is counted, so the swallow is shown to be what held the chip (verified failing with the swallow removed: a boss spawned) — while a finger that moves shows nothing and a plain tap still presses. SEE-THROUGH: the panel ships see-through over live play, ◐ makes it solid and is remembered, and over the pause menu it is always solid with no toggle; and the canvas HUD is clipped out from under the open panel, read off the CANVAS — the minimap's region changes by >80% when the panel opens and again when it closes, against a <20% control between two shut frames (verified failing with the clip removed: 3.5%). |
...
**465 tests** (`npx playwright test --list`). CI runs them in two scopes (user call): every PR push gets the
