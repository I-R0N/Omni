...
| `flashlight.spec.ts` | 5 | **The flashlight is equipment you tap.** The player beam became a KIT-gated in-game tool (user call): tapping your own ship in open space cycles off/medium/high, and the tool exists only while the `flashlight_kit` utility module is installed and hull-adjacent — a kit-less ship carries no beam at all (the DBG flashlight global now ships `off` and stays as the raw dev override under the tool). Pinned: the kit gates the cycle and the renderer override follows it; a REAL tap on the ship cycles the light and fires nothing (the claim eats the gesture); a dock in range still wins the tap (the light is the fallback in the nearest-wins arbitration, not a rival); and uninstalling the kit turns the beam off with it. Gate verified non-vacuous: with it removed, the no-kit test fails. |
