...
| `minimap.spec.ts` | 6 | The minimap's material layer and the portal arrow (step 5). Nebula gone from BOTH halves (the terrain layer is proved blank on the nebula-only showcase); drops excluded in every mode; the streamline cache reused across pans and rebuilt across cells. The shipped material default is DOTS (user call), pinned here because a default is exactly what drifts unwatched. Rewritten for the SCANNER REWORK: the material layer and the portal arrow are both gated on a SCAN now, so what used to be the baseline is asserted as the blank state and each reveal is measured after a real ping. The portal arrow's bracket is the scan's reach rather than `INDICATOR_RANGE`, and a mark is pinned to OUTLIVE the ping that made it — retreating out of reach leaves the arrow up until the mark goes stale, which is the reveal-and-fade model in one assertion. |
| `modules.spec.ts` | 24 | **The Phase-A module families.** PENETRATION: the bonus reaches every gun through one seam, a bolt bores GRAIN BY GRAIN through a body running the boundary model and stops inside when its charges run out, an INDESTRUCTIBLE tile stops it dead and costs it nothing, the falloff RATE is a live knob shipping at 0 (full damage every hit) and reaches all three damage paths a hit produces, and a ricochet may re-hit what it already struck without refreshing the lifetime budget. SCANNER: the baseline is NOTHING on either readout, the home station and the arrival rift are charted anyway (map mark, never an arrow), the mark is a CATEGORY ladder pinned with the same rival at the same distance one mark apart, marks STACK in range but not in category (the user's own Mk III + 2× Mk I worked example, asserted as a ratio), a mark fades and an offline scanner is no scanner, the cooldown refuses the second press, and a fitted-but-never-fired scanner reveals nothing — which is the whole reversal from A4. HEX SLOTS: a locked hex refuses every way in while `HEX_ADJACENCY` never moves, only a station sells the next one, the flower is the cap, and a run reset puts the counts back. |
...
**265 tests.** CI runs them in two scopes (user call): every PR push gets the
