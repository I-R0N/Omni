...
| `healthbars.spec.ts` | 6 | **Damage-triggered health bars** (roadmap 5d, U5 — the parked item). A bar is a HIT REACTION now, not a permanent label, which is behaviour-visible, so what it does is pinned rather than left to a screenshot: an untouched enemy shows nothing, a real shell through `resolveCollision` arms the window, it decays and expires, a fresh hit RE-ARMS rather than accumulating. Plus the two things that MOVED — the player has no world-space bar and `EngineStats.vitals` carries the readout to the HUD every frame; the shield strip is no longer player-only and a shield-ABSORBED hit arms the bar too (which is what makes a drain watchable — the suite caught that gap and it was fixed in the engine, not the test). Plus the opt-out: the dragon keeps a permanent bar, a capstone boss deliberately does not (it has the HUD bar), and the DBG toggle restores the pre-5d always-on behaviour as an honest A/B. |
...
**161 tests.** All but `viewports.spec.ts` run at **390×844** — the phone
