...
test.describe('mass is stated as a density, on one scale', () => {
  test('the shipped hull is exactly the table, and the lean ship is exactly 100',
    async ({ page }) => {
      ...
      // NOTHING RE-PRICED.  The constant was written as a literal 100 and is
      // now `massFor(20, 0.25)`; both halves are asserted so a change to
      // either the size or the density that did NOT intend to move the
      // ship's mass shows up here rather than in a play-test.
      expect(r.derived, 'the shipped constant is still 100').toBe(100);
      expect(r.fromTable, 'and it is the table that produces it').toBe(100);
      expect(r.playerMass, 'a lean ship carries exactly it').toBeCloseTo(100, 6);
      // At index 0 the live density IS the table's — the ladder multiplies,
      // it does not replace.
      expect(r.liveHull).toBe(r.tableHull);
      expect(r.liveHull * r.playerSize * r.playerSize).toBeCloseTo(100, 6);
