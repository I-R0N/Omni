...
/** WHAT AN ITEM DOES, in two lengths — the description popup's text
 *  (`DebugMenu.tsx` / `debugHelp.tsx`: hover with a mouse, long-press on
 *  touch, or rest a keyboard / pad focus on the item).
 *
 *  `summary` is the ONE LINE the popup leads with: what the knob does, in
 *  plain words.  `detail` is the full story behind the popup's "More" — for
 *  every row that had a tooltip before the popup existed, that is the
 *  tooltip, VERBATIM, so nothing written down about a knob was lost when the
 *  one-liners were added.  A row whose old tooltip was already one plain line
 *  carries it as its summary and has no detail.
 *
 *  Both are also what the panel's filter searches in its "Mentioned in
 *  descriptions" tier, so a word that only the long text uses still finds
 *  the row. */
export interface DebugHelp {
  summary?: string;
  detail?: string;
}

/** One chip in a chip row (maps, enemy test, bosses, dragons, rivals).  A
 *  chip is its own item — it has no row label to press — so it carries its
 *  own description. */
export interface DebugChip extends DebugHelp {
  key: string;
  label: string;
  act: (c: DebugCtx) => void;
...
/** One of a `buttons` row's small buttons.  The ROW carries the description:
 *  the buttons are marks of one thing (Hull M1 / M2 / M3). */
export interface DebugButton { label: string; act: (c: DebugCtx) => void }
...
export type DebugRow =
  /** A labelled cycle / toggle: the value IS the button.  Every control has
   *  a summary — a knob nobody can explain is a knob nobody can tune. */
  | { kind: 'ctrl'; label: string; summary: string; detail?: string;
  ...
  | ({ kind: 'stat'; label: string;
      value: (c: DebugCtx) => React.ReactNode; tone?: (c: DebugCtx) => string } & DebugHelp)
  /** A label with a few small buttons (the module Mk grants). */
  | ({ kind: 'buttons'; label: string; buttons: readonly DebugButton[] } & DebugHelp)
  /** A wrap of full-size chips; `label` is what the filter matches.  The
   *  chips describe themselves. */
 ...
 *  to show hierarchy (`  ↳ range`, the timing tree's `·physics`).  Slate-300
 *  rather than the dimmer 400 it was on the opaque panel: over the
 *  see-through backing the world is behind it, and the label is the mark
 *  that has to carry. */
...
/** `ctrl(label, act, value, summary, detail?)` — the old
 *  `ctrlRow(label, onClick, value, title)` with the one-line SUMMARY where the
 *  tooltip was, and the old tooltip (when it was more than one line) moved
 *  after it as the DETAIL.  A call written the old way still reads right:
 *  its tooltip simply becomes the summary. */
const ctrl = (
  label: string,
  act: (c: DebugCtx) => void,
  ...
  summary: string,
  detail?: string,
): DebugRow => ({ kind: 'ctrl', label, act, value, summary, detail });

/** `stat(label, value, summary?, tone?)` — the summary is for readouts whose
 *  label does not say what they measure (most of the timing tree). */
const stat = (
  label: string,
  value: (c: DebugCtx) => React.ReactNode,
  summary?: string,
  tone?: (c: DebugCtx) => string,
): DebugRow => ({ kind: 'stat', label, value, summary, tone });
...
const custom = (
  label: string,
  render: (c: DebugCtx) => React.ReactNode,
  summary?: string,
  detail?: string,
): DebugRow => ({ kind: 'custom', label, render, summary, detail });
...
/** A map picker entry: the map, its chip label, and one line on what it is. */
export interface MapChoice { type: MapType; label: string; summary: string }
...
export const REAL_MAPS: readonly MapChoice[] = [
  // Wave-free home map: station POIs + ambient roamers.  No waves.
  { type: MapType.OVERWORLD,   label: 'Overworld',
    summary: 'The wave-free home hub: stations to dock at and a rift to every arena.' },
  { type: MapType.UNIVERSE,    label: 'Deep Space',
    summary: 'The biggest arena (16k): open space with tile clusters, asteroids and nebulae.' },
  { type: MapType.RING,        label: 'Ring World',
    summary: 'An arena with one ring of glass tiles around the spawn, under a circular flow.' },
  { type: MapType.SEVEN_RINGS, label: 'Seven Rings',
    summary: 'An arena of seven concentric tile rings, under a circular flow.' },
  { type: MapType.POCKET,      label: 'Pocket',
    summary: 'A small 4k arena with every element packed close together.' },
];

/** The single-element showcase maps (plus the multi-material Tile Heavy
 *  stress map). */
export const TEST_MAPS: readonly MapChoice[] = [
  { type: MapType.ASTEROID_FIELD,       label: 'Asteroid Field',
    summary: 'Asteroids only, riding one giant vortex — no tiles, no nebulae.' },
  { type: MapType.GLASS_FIELD,          label: 'Glass Field',
    summary: 'Glass tiles only.' },
  { type: MapType.PLASTIC_FIELD,        label: 'Plastic Field',
    summary: 'Plastic tiles only.' },
  { type: MapType.METAL_FIELD,          label: 'Metal Field',
    summary: 'Metal tiles only — the toughest material that still breaks.' },
  { type: MapType.INDESTRUCTIBLE_FIELD, label: 'Indestructible',
    summary: 'Permanent wall clusters only: a maze that never breaks.' },
  { type: MapType.NEBULA_FIELD,         label: 'Nebula Field',
    summary: 'Pass-through nebula clouds only.' },
  { type: MapType.ROCK_FIELD,           label: 'Rock Field',
    summary: 'Rock tiles only; broken rock drifts off as asteroids.' },
  { type: MapType.TILE_HEAVY,           label: 'Tile Heavy',
    summary: 'Dense clusters of every tile material — a tile-rendering stress test.' },
];
...
export const ENEMY_TEST: readonly { type: EnemySubtype | null; label: string; summary: string }[] = [
  { type: null,                   label: 'Off',
    summary: 'Normal waves: the usual mix of enemies, with ambient bubbles.' },
  { type: EnemySubtype.RAMMER_1,  label: 'Drone',
    summary: 'A fast, frantic diver spraying weak pellets.' },
  { type: EnemySubtype.RAMMER_2,  label: 'Charger',
    summary: 'A strafing dasher with a two-shot fan.' },
  { type: EnemySubtype.RAMMER_3,  label: 'Tank',
    summary: 'A slow, armoured siege slug — chip fire barely scratches it.' },
  { type: EnemySubtype.SHOOTER_1, label: 'Skirmisher',
    summary: 'The baseline kiter: keeps its distance and fires single bolts.' },
  { type: EnemySubtype.SHOOTER_2, label: 'Orbiter',
    summary: 'Circles you spitting acid rounds that corrode your hull over time.' },
  { type: EnemySubtype.SHOOTER_3, label: 'Sniper',
    summary: 'Holds still, paints you with a laser, then fires one fast, heavy shot.' },
  { type: EnemySubtype.KAMIKAZE,  label: 'Kamikaze',
    summary: 'Dives in and explodes on contact — kill it before it reaches you.' },
  { type: EnemySubtype.BULWARK,   label: 'Bulwark',
    summary: 'A slow fortress behind a turning arc shield; get around it.' },
  { type: EnemySubtype.TURRET,    label: 'Turret',
    summary: 'A stationary emplacement lobbing slow homing missiles.' },
  { type: EnemySubtype.SWARM,     label: 'Swarm',
    summary: 'Cheap one-hit gnats that flock in and pop on contact.' },
  { type: EnemySubtype.NEST,      label: 'Nest',
    summary: 'A static hive that keeps birthing Swarm gnats until destroyed.' },
];
const ENEMY_TEST_DETAIL =
  'Forces EVERY wave to spawn only this type (DBG), for testing one enemy in isolation. The ambient bubbles are held off while a type is forced. Off restores normal waves.';

/** Capstone warp-ins, each click stacking another (the Dragon-menu
 *  pattern). */
const BOSS_SPAWNS: readonly { k: string; label: string; summary: string }[] = [
  { k: 'BOSS_WARDEN', label: 'Warden',
    summary: 'A slow, shielded bastion that shells you from mid-range; later it loses its shield and plating and calls a Swarm escort.' },
...
const DRAGON_DETAIL =
  'Enters through a rift, roams, and grows by eating terrain — its body IS the tiles it ate, and shooting a segment severs everything behind it. Passive until attacked. Each click stacks another (DBG).';

const RIVAL_SPAWNS: readonly { k: string; c: string; summary: string }[] = [
  { k: 'hostile', c: 'hover:border-red-400',
    summary: 'A red privateer that fights you and the enemies alike.' },
  { k: 'ally', c: 'hover:border-emerald-400',
    summary: 'A green privateer that fights enemies only, never you.' },
  { k: 'neutral', c: 'hover:border-amber-400',
    summary: 'An amber privateer that hunts enemies for loot and ignores you — until you shoot it.' },
  { k: 'random', c: 'hover:border-sky-400',
    summary: 'A privateer with its side rolled at the normal odds.' },
];
const RIVAL_DETAIL =
  'Warps in through a rift. A rival steals the kill points of the enemies it downs and vacuums up nearby salvage; you can shoot it down for a bounty. Each click stacks another (DBG).';
 ...
 *  into the inventory (auto-installs if a compatible hex is free).  The
 *  buttons used to carry one tooltip each saying the same thing with the mark
 *  swapped in; the row now says it once. */
const markRow = (fam: string, label: string, marks: readonly number[]): DebugRow => ({
  kind: 'buttons',
  label,
  summary: `Grant a ${label} module at the mark you tap (M1–M${marks[marks.length - 1]}).`,
  detail: `Each button grants that mark of ${label} into the inventory (DBG). Auto-installs if a compatible hex is free — modules are fixed items, no levels.`,
...
const mapChips = (label: string, maps: readonly MapChoice[], detail: string): DebugRow =>
...
/** What picking a map DOES — the same for every map in a list, so it rides
 *  behind "More" while each map's own summary says what the map is. */
const MAP_PICK_DETAIL =
  'Picking a map is a DEBUG override that lasts the run it starts. From the main menu it only swaps the backdrop START will use; anywhere else it restarts the run on that map — credits, outfit and score reset.';
const FIELD_MAP_DETAIL =
  'A 6k showcase map holding ONE kind of element, so a single system (flow, regen, fracture, lighting) can be judged on its own; waves still run. ' + MAP_PICK_DETAIL;
...
export const DEBUG_SECTIONS: readonly DebugSection[] = [
  {
    ...
    rows: [
      ...
      ctrl('Speed', dbg(e => e.dbg.cyclePlayerSpeed()),
        ...
        'Caps the ship\'s top speed; only bites when the cap is below cruise speed.',
  ...
  },
  {
    ...
    rows: [
      ctrl('Impact vel', dbg(e => e.dbg.cycleImpactVelocity()),
        ...
        'Which velocity a hit\'s damage is measured in: the bolt\'s own (muzzle) or the closing speed (relative).',
  ...
  },
  {
    ...
    rows: [
      ctrl('Roll feel', dbg(e => e.dbg.cyclePlayerRoll()),
        ...
        'How far the hull visibly leans into turns and thrust (ships Off). Purely visual.',
      ...
      ctrl('Hull', dbg(e => e.dbg.cyclePlayerHull()),
        ...
        'What draws as the player\'s ship: the sprite, pre-rendered tilt art, or a wireframe shape.',
      ...
      ctrl('Roll damp', dbg(e => e.dbg.cycleRollDamping()),
        ...
        'How quickly the tilt spring settles — looser or tighter, with the same wobble.',
      ...
      ctrl('Tilt mode', dbg(e => e.dbg.cycleTiltMode()),
        ...
        'Lean tilts toward the acceleration and settles back; Tumble rolls continuously with thrust (a test).',
      ...
      ctrl('Lean dir', dbg(e => e.dbg.cycleLeanDir()),
        ...
        'Flips which way the hull leans in Lean mode (A/B).',
      ...
      ctrl('Tilt src', dbg(e => e.dbg.cycleTiltSource()),
        ...
        'What drives the tilt: your thrust input, your actual velocity, or both blended.',
      ...
      ctrl('Vel gain', dbg(e => e.dbg.cycleVelGain()),
        c => c.s.velGainName ?? '1×',
        'Sensitivity of the Velocity tilt source — higher steps reach full tilt at lower speeds.',
  ...
  },
  {
    ...
    rows: [
      ...
      ctrl('Disable', dbg(e => e.debugApplyDisable()),
        c => 'EMP',
        'EMP yourself: your guns can\'t fire and your shield goes offline for a few seconds.',
  ...
  },
  {
    ...
    rows: [
      stat('Gamepad', c => c.s.gamepadInfo ?? 'none', 'The gamepad the game adopted, or none.', c => (c.s.gamepadInfo && c.s.gamepadInfo !== 'none') ? 'text-sky-300' : 'text-slate-400'),
      stat('  ↳ axes', c => c.s.gamepadAxes ?? '—', 'Live thrust, aim heading and fire state, as they reach the game.', c => 'text-slate-400'),
      stat('  ↳ rumble', c => c.s.rumbleInfo ?? '—', 'Why rumble is or isn\'t available right now (ready, playing, no pad…).', c => c.s.rumbleInfo === 'ready' || c.s.rumbleInfo === 'playing' ? 'text-emerald-300' : 'text-slate-400'),
      stat('  ↳ triggers', c => c.s.adaptiveTriggerInfo ?? '—', 'Adaptive-trigger link (DualSense over WebHID): unsupported, not connected, or the transport in use.', c => c.s.adaptiveTriggersConnected ? 'text-emerald-300' : 'text-slate-400'),
      stat('  ↳ report', c => c.s.adaptiveTriggerReport ?? '—', 'The start of the last adaptive-trigger report sent to the pad, in hex.', c => 'text-slate-400'),
      ctrl('  ↳ trig enc', dbg(e => e.cycleTriggerEncoding()),
        c => c.s.adaptiveTriggerInfo?.includes('simple') ? 'simple' : 'zones',
        'Which wire encoding the adaptive-trigger effects use (zones or simple) — try the other if the triggers feel limp.',
      ...
      ctrl('  ↳ HID buzz', dbg(e => e.testAdaptiveTriggerLink()),
        c => 'Test',
        'Buzzes the pad\'s motors over HID, to test the link separately from the trigger encoding.',
      ...
      ctrl('Rumble', dbg(e => e.dbg.toggleRumble()),
        ...
        'Gamepad rumble on impacts (it follows the screen shake), on or off.',
      ...
      ctrl('Joystick', dbg(e => e.dbg.toggleJoystickDebug()),
        ...
        'Forces the touch joystick to draw, so its size and placement can be checked on a desktop.',
  ...
  },
  {
    ...
    rows: [
      ...
      ctrl('Light', dbg(e => e.debugGrantModule('flashlight_kit')),
        c => 'Grant',
        'Grant the Light module; then tap your ship in open space to cycle the light off / medium / high.',
  ...
  },
  {
    ...
    rows: [
      ...
      ctrl('Lock slots', dbg(e => e.dbg.cycleSlotLock()),
        c => c.s.debugSlotLock ?? '—',
        'Locks hex slots on both flowers, so the station\'s "+1 Hex Slot" purchase can be tested.',
  ...
  },
  {
    ...
    rows: [
      mapChips('Maps', REAL_MAPS, MAP_PICK_DETAIL),
    ],
  },
  {
    id: 'fieldmaps', label: 'Material Field Maps', group: 'world',
    rows: [
      mapChips('Material Field Maps', TEST_MAPS, FIELD_MAP_DETAIL),
    ],
  },
  {
  ...
  },
  {
    ...
    rows: [
      ctrl('Pattern', dbg(e => e.dbg.cycleFFPattern()),
        ...
        'The shape of the current that asteroids ride: the map\'s own, or spiral, well, waves….',
      ...
      ctrl('Ast flow', dbg(e => e.dbg.toggleShardFlow()),
        ...
        'Whether the flow current pushes asteroids and shards at all.',
      ...
      ctrl('Density', dbg(e => e.dbg.cycleFFDensity()),
        ...
        'Flow-field cell size: finer cells bend the flow more precisely, but shorten enemy pathfinding reach.',
      ...
      ctrl('Kernel R', dbg(e => e.dbg.cycleFFKernelR()),
        c => `R=${c.s.ffKernelR ?? 3}`,
        'How early the flow starts bending away from walls.',
      ...
      ctrl('Tangent', dbg(e => e.dbg.cycleFFTangentMix()),
        ...
        'Whether the flow meets a wall by pushing straight off it or sliding along it.',
      ...
      ctrl('Breathe', dbg(e => e.dbg.cycleFFBreathe()),
        ...
        'Slowly shifts the flow over time, so piles of shards drift apart.',
      ...
      ctrl('Lane', dbg(e => e.dbg.cycleFFLaneJitter()),
        ...
        'Spreads shards into parallel lanes instead of one stream.',
      ...
      ctrl('Vec overlay', dbg(e => e.dbg.toggleFFOverlayVectors()),
        ...
        'Draws the flow direction as arrows over the world.',
      ...
      ctrl('Obstacles', dbg(e => e.dbg.toggleFFOverlayObstacles()),
        ...
        'Tints the cells the flow field treats as blocked.',
      ...
      ctrl('Rebuilds', dbg(e => e.dbg.toggleFFOverlayRebuilds()),
        ...
        'Flashes cells as the flow field re-bakes them after a tile is destroyed.',
  ...
  },
  {
    ...
    rows: [
      ctrl('Enemy scale', dbg(e => e.dbg.cycleEnemyScale()),
        ...
        'How fast enemy HP and damage grow wave to wave (0 = no growth).',
      ...
      stat('  ↳ live', c => c.s.enemyScaleInfo ?? '—', 'The HP and damage multipliers enemies spawn with at this wave.', c => 'text-slate-400'),
      ctrl('Traits', dbg(e => e.toggleTraits()),
        c => c.s.traitsEnabled === false ? 'Off' : 'On',
        'Enemy counterplay traits — armour, shields, evasion, regen — on or off.',
  ...
  },
  {
    ...
    rows: [
      ctrl('Snitch catch', dbg(e => e.dbg.toggleSnitchCatchMode()),
        ...
        'Catch the golden snitch by flying into it, or by shooting it.',
      ...
      ctrl('Snitch spd', dbg(e => e.dbg.cycleSnitchSpeed()),
        ...
        'Scales the snitch\'s speed (it gets faster each time one is caught).',
  ...
  },
  {
    ...
    rows: [
      chips('Warp in a boss', () => BOSS_SPAWNS.map(({ k, label, summary }) => ({
        key: k, label, summary,
        act: dbg(e => e.debugSpawnBoss(k)),
        detail: 'Warp this boss in with its full phase table (DBG). Each click stacks another.',
  ...
  },
  {
    ...
    rows: [
      chips('Summon a dragon', () => DRAGON_SPAWNS.map(t => ({
        ...
        summary: t === 'mixed'
          ? 'Summon a dragon whose starting body mixes every material.'
          : `Summon a dragon whose starting body is ${t} tiles.`,
        detail: DRAGON_DETAIL,
  ...
  },
  {
    ...
    rows: [
      chips('Warp in a rival', () => RIVAL_SPAWNS.map(({ k, c: hover, summary }) => ({
        key: k, label: k, capitalize: true,
        summary, detail: RIVAL_DETAIL,
  ...
  },
  {
    ...
    rows: [
      ctrl('Fracture', dbg(e => e.dbg.cycleFractureMode()),
        ...
        'How breakable materials shatter: along their own Voronoi grain (default), or the old random-fragment model.',
      ...
      ctrl('Frac relax', dbg(e => e.dbg.cycleFractureRelax()),
        ...
        'How regular grain cells are, forced on every material (default: each material\'s own).',
      ...
      ctrl('Frac sep', dbg(e => e.dbg.cycleFractureSeparation()),
        ...
        'Minimum spacing between grain sites, forced on every material.',
      ...
      ctrl('Frac sites', dbg(e => e.dbg.cycleFractureSiteScale()),
        ...
        'More, smaller pieces or fewer, bigger ones — a multiplier on grain count.',
      ...
      ctrl('Frac bias', dbg(e => e.dbg.cycleFractureBias()),
        ...
        'How much the grain pattern crowds toward the impact point.',
      ...
      ctrl('Chip dust', dbg(e => e.dbg.cycleChipDustPool()),
        ...
        'How many chips\' worth of dust pool into one puff — bigger, rarer puffs as it rises.',
      ...
      ctrl('Grain mat', dbg(e => e.dbg.cycleGrainMaterial()),
        ...
        'Which material the ↳ rows below edit (its tile and shard together).',
      ...
      ctrl('  ↳ grain size', dbg(e => e.dbg.cycleGrainKnob('grainSize')),
        ...
        'Grain diameter for the selected material — smaller grains are finer and tougher.',
      ...
      ctrl('  ↳ count min', dbg(e => e.dbg.cycleGrainKnob('grainCountMin')),
        ...
        'The fewest grains a body of this material may have.',
      ...
      ctrl('  ↳ count max', dbg(e => e.dbg.cycleGrainKnob('grainCountMax')),
        ...
        'The most grains a body of this material may have (a performance cap).',
      ...
      ctrl('  ↳ regularity', dbg(e => e.dbg.cycleGrainKnob('regularity')),
        ...
        'How even this material\'s grain pattern is, from 0 (ragged) to 1 (honeycomb).',
      ...
      ctrl('  ↳ dmg spread', dbg(e => e.dbg.cycleGrainKnob('damageSpread')),
        ...
        'Damage spread for this material alone.',
  ...
  },
  {
    ...
    rows: [
      ctrl('Neb sprite', dbg(e => e.dbg.cycleNebulaSpriteSize()),
        ...
        'How far a nebula cloud\'s sprite overhangs its body — the size of the clouds.',
      ...
      ctrl('Neb damp', dbg(e => e.dbg.cycleNebulaDamp()),
        ...
        'How quickly drifting nebula puffs slow down.',
      ...
      ctrl('Neb spin damp', dbg(e => e.dbg.cycleNebulaSpinDamp()),
        ...
        'How quickly nebula puffs stop spinning.',
      ...
      ctrl('Neb stretch', dbg(e => e.dbg.cycleNebulaStretch()),
        ...
        'How much moving nebula puffs stretch along their travel.',
      ...
      ctrl('Neb spin', dbg(e => e.dbg.cycleNebulaWakeSpin()),
        ...
        'Which way your wake spins the nebula you pass.',
      ...
      ctrl('Neb collide', dbg(e => e.dbg.toggleNebulaShardCollisions()),
        ...
        'Makes nebula puffs bounce off each other (normally they pass through).',
      ...
      ctrl('Nebula', dbg(e => e.dbg.cycleNebulaPalette()),
        ...
        'Colour palette of the dust glass throws when it shatters or merges.',
  ...
  },
  {
    ...
    rows: [
      ctrl('Local grav', dbg(e => e.dbg.toggleLocalGravity()),
        ...
        'The player\'s local gravity on nearby asteroids, on or off.',
      ...
      ctrl('Attract grav', dbg(e => e.dbg.toggleAttractorGravity()),
        ...
        'Gravity wells (portals and other attractors), on or off.',
      ...
      ctrl('Tile push', dbg(e => e.dbg.toggleRepelPush()),
        ...
        'Glass and metal tiles pushing nearby ships away.',
      ...
      ctrl('Shard grav', dbg(e => e.dbg.toggleShardGravity()),
        ...
        'Shards pulling on each other (today only nebula does).',
      ...
      ctrl('Bonding', dbg(e => e.dbg.toggleShardBonding()),
        ...
        'Shards sticking together and merging.',
      ...
      ctrl('Plr↔neb', dbg(e => e.dbg.togglePlayerNebulaCollision()),
        ...
        'Whether your ship physically scatters nebula, or glides through it.',
      ...
      ctrl('Sleep', dbg(e => e.dbg.toggleShardSleep()),
        ...
        'Lets resting shards stop colliding until something disturbs them (performance).',
      ...
      ctrl('Vp cull', dbg(e => e.dbg.toggleShardViewportCull()),
        ...
        'Resolves off-screen shard pairs less often (performance).',
      ...
      ctrl('Shard LOD', dbg(e => e.dbg.toggleShardLod()),
        ...
        'Draws tiny shards as simple discs (performance; visual only).',
      ...
      ctrl('Merge rate', dbg(e => e.dbg.toggleMergeRate()),
        ...
        'Faster merging in dense pockets of shards.',
      ...
      ctrl('Grace', dbg(e => e.dbg.cycleShatterGrace()),
        ...
        'How long fresh debris stays scattered before it may re-condense.',
      ...
      ctrl('Tile blend', dbg(e => e.dbg.cycleTileBlendAlpha()),
        ...
        'How fast neighbouring nebula tiles blend their colours.',
      ...
      ctrl('Shard blend', dbg(e => e.dbg.cycleShardBlendAlpha()),
        ...
        'How fast nebula puffs take on the colour of the nearest tile.',
      ...
      ctrl('Blend int', dbg(e => e.dbg.cycleColorBlendInterval()),
        ...
        'How often the colour blending runs (auto or pinned; performance).',
  ...
  },
  {
    ...
    rows: [
      ctrl('Rock palette', dbg(e => e.dbg.cycleRockPalette()),
        ...
        'Rock colour family: mixed, slate, rust or mineral (applies to rock that spawns after).',
      ...
      ctrl('Tile shade', dbg(e => e.dbg.toggleMaterialAutomata()),
        ...
        'Shades tiles in dense clusters by how many neighbours they have.',
      ...
      ctrl('Pl shade', dbg(e => e.dbg.togglePlasticAutomata()),
        ...
        'Shades plastic shards by how many others they touch.',
      ...
      ctrl('P glow', dbg(e => e.dbg.cyclePlasticGlowBrightness()),
        ...
        'How brightly plastic tiles glow when something comes near.',
      ...
      ctrl('Recolor', dbg(e => e.dbg.togglePlasticBlend()),
        ...
        'Plastic colours blending over time (off freezes them).',
      ...
      ctrl('Goo bond', dbg(e => e.dbg.toggleShardBlend()),
        ...
        'Draws stuck-together plastic as one gooey blob (visual only).',
      ...
      ctrl('Goo coat', dbg(e => e.dbg.cycleShardCoat()),
        ...
        'Thickness of the goo coat on bonded plastic.',
  ...
  },
  {
    ...
    rows: [
      ...
      ctrl('Fog', dbg(e => e.dbg.cycleFog()),
        c => c.s.fogName ?? 'off',
        'Fog of war that your light cuts through: dim, dark, or with memory of where you have been.',
      ...
      ctrl('Emit fade', dbg(e => e.dbg.cycleEmitFade()),
        ...
        'How smoothly glowing bodies fade in and out.',
      ...
      ctrl('Emit shadow', dbg(e => e.dbg.toggleEmitShadows()),
        ...
        'Lets glowing bodies cast shadows too (costly).',
      ...
      ctrl('Emit shd tier', dbg(e => e.dbg.cycleEmitShadowTier()),
        ...
        'How many glowing bodies cast shadows, and in how much detail.',
      ...
      ctrl('Shard shadows', dbg(e => e.dbg.toggleShardShadows()),
        ...
        'Mobile shards cast shadows too, not just tiles.',
      ...
      ctrl('Refraction', dbg(e => e.dbg.toggleRefraction()),
        ...
        'Glass bends light into a caustic cone instead of passing it straight through.',
      ...
      ctrl('Caustic fade', dbg(e => e.dbg.cycleCausticFade()),
        ...
        'Smooths caustics appearing and vanishing as glass turns.',
      ...
      ctrl('Refr bright', dbg(e => e.dbg.cycleRefractBrightness()),
        ...
        'Brightness of the refracted light.',
  ...
  },
  {
    ...
    rows: [
      ctrl('Star density', dbg(e => e.dbg.cycleStarDensity()),
        ...
        'Stars per screen area; Auto uses this map\'s own sky.',
      ...
      ctrl('Parallax', dbg(e => e.dbg.cycleStarParallax()),
        ...
        'How much faster the nearest stars scroll than the farthest.',
  ...
  },
  {
    ...
    rows: [
      ...
      ctrl('Chevrons', dbg(e => e.dbg.toggleChevronMode()),
        ...
        'Whether the edge arrows also mark contacts already on screen.',
  ...
  },
  {
    ...
    rows: [
      ...
      stat('Wave', c => c.s.waveNumber ?? 1, 'The current wave within this stage.'),
      stat('State', c => c.s.waveStatus ?? '—', 'What the wave system is doing right now.'),
      ...
      }, 'Seconds left in the wave. Unused since waves became clear-the-field, so it always reads —.'),
      ...
      }, 'How many entities exist; the menu beside it counts all of them, the awake ones, or the asleep ones.',
      'Entity-count filter (display only): total = all entities, active = awake, asleep = dynamic-sleeping (count of resting shards). Sleeping behaviour itself is the Shards & Physics ▸ Sleep toggle.'),
    ],
  },
  {
    id: 'perfstats', label: 'Perf', group: 'perf', when: c => !!c.s.perf,
    rows: [
      stat('enemies', c => c.s.perf!.enemyCount, 'Enemies alive right now.'),
      stat('asteroids', c => c.s.perf!.mobileShardCount, 'Mobile shards alive (asteroids and loose debris).'),
      stat('projectiles', c => c.s.perf!.projectileCount, 'Shots in flight.'),
      stat('particles', c => c.s.perf!.particleCount, 'Live particles.'),
      stat('drops/POI', c => c.s.perf!.interactableCount, 'Drops, portals, stations and other interactables.'),
      stat('max cell', c => c.s.perf!.maxCellDensity, 'The most bodies in any one collision cell — 10+ is crowded, 20+ is heavy.', c => c.s.perf!.maxCellDensity >= 20 ? 'text-red-400' : c.s.perf!.maxCellDensity >= 10 ? 'text-amber-300' : 'text-white'),
      ctrl('Auto', dbg(e => e.dbg.togglePerfAuto()),
        c => c.s.perfAutoEnabled === false ? 'Off' : 'On',
        'Lets the performance controller skip work under load.',
      ...
      stat('load',
        c => `${c.s.perf!.perfLoadTier} (${Math.round(c.s.perf!.perfLoadLevel * 100)}%)`, 'The load tier and level driving the automatic throttle.',
      ...
      }, 'Moving entities — the load the throttle reacts to. In brackets: how many are asleep, off-screen, or drawn as LOD discs.'),
      custom('merge rate', c => {
        const perf = c.s.perf!;
      ...
      }, 'How much faster shards merge right now, from local density (see Shards & Physics ▸ Merge rate).'),
      ...
      each('perf tasks', c => (c.s.perf?.perfTasks ?? []).map(t =>
        stat(`\u00a0·${t.id}`, () => `${t.eff}${t.manual >= 1 ? '!' : ''}`,
          `How often the ${t.id} pass runs: 1 = every sim step, higher = skipped under load; ! = pinned by hand.`,
  ...
  },
  {
    ...
    rows: [
      stat('updPhys', c => fmtMs(c.s.perf!.updatePhysicsMs), 'Average ms per step in updatePhysics: AI, the flow field, and the physics step.'),
      stat(' ·physics', c => fmtMs(c.s.perf!.physicsMs), 'The physics step: gravity, collisions and movement.'),
      stat('  ·grav', c => fmtMs(c.s.perf!.gravityMs), 'Gravity wells (portals and other attractors).'),
      stat('  ·lgrv', c => fmtMs(c.s.perf!.localGravityMs), 'The player\'s local gravity on nearby asteroids.'),
      stat('  ·coll', c => fmtMs(c.s.perf!.collisionsMs), 'Collision broadphase and resolution.'),
      stat(' ·ai', c => fmtMs(c.s.perf!.aiMs), 'Enemy AI.'),
      stat(' ·flow', c => fmtMs(c.s.perf!.flowFieldMs), 'Updating the enemy pursuit flow field.'),
      stat(' ·misc', c => fmtMs(c.s.perf!.physMiscMs), 'The rest of updatePhysics: entity bookkeeping, the asteroid census, flow nudges.'),
      stat('updLogic', c => fmtMs(c.s.perf!.updateLogicMs), 'Average ms per step in updateGameLogic: shards, waves, weapons, drops and the rest.'),
      stat(' ·shards', c => fmtMs(c.s.perf!.shardSysMs), 'The shard system: merging, bonding and regen.'),
      stat(' ·rings', c => fmtMs(c.s.perf!.explosionRingsMs), 'Explosion shockwave rings.'),
      ...
      stat(' ·neb fast/slow', c => `${c.s.perf!.nebulaFast}/${c.s.perf!.nebulaSlow}`, 'Nebula bodies drawn from cache (fast) vs recomputed (slow).'),
      stat(' ·tLit', c => fmtMs(c.s.perf!.tileLightingMs), 'The legacy tile-glow lighting.'),
      stat(' ·tLit-N', c => c.s.perf!.tileLightingCount, 'Tiles lit by the legacy glow this frame.'),
      stat(' ·lit', c => fmtMs(c.s.perf!.lightingMs), 'The unified lighting pass.'),
      stat(' ·lit-N', c => c.s.perf!.lightingLights, 'Lights in the unified pass this frame.'),
      stat(' ·fog', c => fmtMs(c.s.perf!.fogMs), 'The fog-of-war layer (0 while fog is off).'),
    ],
  },
  {
    ...
    rows: [
      ...
      ), 'Record frame rate and timing over a stretch of play, then export it as a report.',
      '○ REC starts / stops an FPS + perf capture. The chip beside it cycles the scene label recorded with the capture. Copy exports the capture as a copy-paste report — to the clipboard, and into a box below it (tap it to select all, then copy), which is the fallback where the clipboard refuses.'),
    ],
  },
  {
    ...
    rows: [
      ctrl('Sim rate', dbg(e => e.dbg.cycleSimRate()),
        ...
        'Simulation steps per second: 120Hz (default) or 60Hz — the biggest lever on sim cost.',
      ...
      ctrl('Substep cap', dbg(e => e.dbg.cycleSubstepCap()),
        ...
        'The most sim steps one frame may catch up on — lower stops a slow frame snowballing.',
      ...
      ctrl('Render scale', c => c.app.cycleRenderScale(),
        ...
        'Caps the canvas resolution (device-pixel ratio): lower is softer, but cheaper.',
      ...
      ctrl('HUD rate', dbg(e => e.dbg.cycleHudRate()),
        ...
        'How often the HUD overlay re-renders (60 / 30 / 15Hz).',
  ...
  },
  {
    ...
    rows: [
      ctrl('Overlays', dbg(e => e.dbg.toggleDebug()),
        ...
        'Draws collision shapes and the input vector over the world.',
      ...
      ctrl('Outlines', dbg(e => e.dbg.toggleTileOutlines()),
        ...
        'Outlines the collision shape on plastic and nebula.',
